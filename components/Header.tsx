...
export function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 border-b border-black/5 bg-[#fffaf4]/90 backdrop-blur-xl dark:border-white/10 dark:bg-[#181614]/90 transition-transform duration-300 ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
      <div className="container-premium py-3 md:h-16 md:py-0 flex flex-col md:flex-row justify-center md:justify-between gap-3 md:gap-4">
        <div className="flex items-center justify-between w-full md:w-auto">
          <Link href="/" className="flex items-center gap-2 font-serif text-lg tracking-normal text-ink whitespace-nowrap dark:text-linen">
            <Image src="/logo.jpg" alt="Vikore Vana Logo" width={32} height={32} className="rounded-full object-cover shrink-0" />
            Vikore Vana
          </Link>
        ...
        <div className="flex items-center gap-5 md:gap-7 overflow-x-auto no-scrollbar justify-start md:justify-center w-full md:w-auto">
          {site.nav.map((item) => (
            <Link key={item.href} href={item.href} className="text-sm font-medium whitespace-nowrap text-smoke transition hover:text-ink dark:text-bone dark:hover:text-white">
              {item.label}
            </Link>
          ))}
        </div>
