...
export function Header() {
  ...
  // Mobile-only: show dark mode button while scrolling, hide after idle
  useEffect(() => {
    ...
    <>
            ...
            {site.nav.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative py-1 text-[10px] font-medium uppercase tracking-[0.3em] transition-colors duration-300 ${active
      ...
      <button
        ...
        className={`fixed bottom-10 sm:bottom-16 right-4 sm:right-6 z-[99] flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-full
          ...
          text-ink dark:text-linen hover:scale-110 active:scale-95 transition-all duration-500
          sm:opacity-100 sm:pointer-events-auto
          ${darkBtnVisible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none sm:opacity-100 sm:pointer-events-auto'}`}
      >
                    ...
                    return (
                      <motion.div key={item.href} variants={itemVariants}>
                        <Link
                          ...
                          className={`group flex items-center justify-between py-2 text-lg sm:text-xl font-serif font-light tracking-[0.08em] uppercase transition-all duration-300 ${active
                              ? "text-[#b89569] dark:text-[#cba677]"
                        ...
                        >
                          ...
                          <ArrowRight
                            size={16}
                            className={`text-[#b89569] dark:text-[#cba677] opacity-0 -translate-x-2 transition-all duration-300 ${isHovered ? "opacity-100 translate-x-0" : ""
                              }`}
                          />
                        </Link>
                      </motion.div>
