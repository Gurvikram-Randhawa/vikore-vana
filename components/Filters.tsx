...
export function FilteredProducts({ products }: { products: Product[] }) {
  ...
  return <FilterShell query={query} setQuery={setQuery} category={category} setCategory={setCategory} gridClassName="grid gap-3 sm:gap-4 md:gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">{filtered.map((product) => <ProductCard key={product.slug} product={product} />)}</FilterShell>;
}
